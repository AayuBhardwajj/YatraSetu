...
import {
  User as UserIcon, Mail, Phone, MapPin, Home, Briefcase,
  Edit2, Check, LogOut, Camera, ChevronRight, ShieldCheck,
  Star, Bell, CreditCard, History, Lock, Smartphone, Plus,
} from 'lucide-react'
...
export default function ProfilePage() {
  ...
  const { user, userProfile, updateUserProfile, signOut, isLoading } = useAuthStore()
  const [isEditing, setIsEditing] = useState(false)
  const [isSaving, setIsSaving] = useState(false)
  const [form, setForm] = useState({ name: '', email: '', phone: '', location: '' })

  // Populate form from cached store — no Supabase call needed
  useEffect(() => {
    if (user || userProfile) {
      setForm({
        ...
        location: (userProfile as any)?.city || '',
      })
    }
  }, [user, userProfile])
  ...
  const saveProfile = async () => {
    ...
    await Promise.all([
      supabase.from('user_profiles').update({ phone: form.phone, city: form.location, full_name: form.name }).eq('user_id', user.id),
      supabase.auth.updateUser({ data: { full_name: form.name } }),
    ])

    // Optimistic update — store reflects change instantly, no re-fetch
    updateUserProfile({ full_name: form.name, phone: form.phone })
    setIsSaving(false)
    setIsEditing(false)
  }
    ...
    return (
      <div className="max-w-[1100px] mx-auto p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-5"><Skeleton className="h-[480px] rounded-3xl" /></div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="p-6 border-none bg-white rounded-3xl shadow-lg shadow-black/[0.02] flex flex-col items-center text-center relative overflow-hidden group">
            ...
            <div className="mt-5 space-y-2">
              <h2 className="text-xl font-bold text-text-primary tracking-tight">{displayName}</h2>
        ...
        <div className="lg:col-span-7 space-y-8">
          <section className="space-y-4">
            <div className="flex items-center justify-between px-1">
              ...
              >
                {isEditing
                  ? <>{isSaving ? 'Saving...' : <><Check className="w-3.5 h-3.5 mr-1.5" />Save</>}</>
            ...
            <Card className="p-7 border-none bg-white rounded-3xl shadow-lg shadow-black/[0.02] space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {[
                  { label: 'Full Name', icon: UserIcon, key: 'name' as const },
                  { label: 'Email', icon: Mail, key: 'email' as const },
                  { label: 'Phone', icon: Smartphone, key: 'phone' as const },
                  { label: 'Location', icon: MapPin, key: 'location' as const },
                ].map(({ label, icon: Icon, key }) => (
                  <div key={key} className="space-y-1.5">
                    <label className="text-[11px] font-bold text-text-muted uppercase tracking-wider ml-1">{label}</label>
