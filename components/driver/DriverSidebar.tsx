...
import {
  LayoutDashboard, MapPin, IndianRupee,
...
export function DriverSidebar() {
  ...
  const { signOut, user, driverProfile } = useAuthStore()

  const displayName = driverProfile?.full_name || user?.user_metadata?.full_name || 'Driver'
      ...
      <div className="p-6 space-y-8">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-xl font-bold text-text-primary tracking-tight">
            Zipp <span className="text-success text-xs font-bold uppercase ml-1 tracking-widest">Driver</span>
          </span>
        </div>
        ...
        <div className={cn(
          ...
          <div className="flex items-center justify-between">
            ...
            <Switch checked={isOnline} onCheckedChange={setIsOnline} className="data-[state=checked]:bg-success" />
          </div>
          <p className="text-[11px] font-medium text-text-muted leading-relaxed">
            {isOnline ? "You're visible to riders and can receive ride requests." : 'Go online to start receiving requests.'}
          </p>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? 'bg-success-light text-success' : 'text-text-secondary hover:bg-gray-50'
              )}
            >
              {isActive && <div className="absolute left-0 top-0 bottom-0 w-[4px] bg-success rounded-r-full" />}
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center space-x-3 p-2 bg-muted/50 rounded-xl">
          ...
          <div className="flex flex-col truncate">
            ...
            <div className="flex items-center text-[10px] text-warning font-bold">★ 4.8 Rating</div>
          </div>
        </div>
